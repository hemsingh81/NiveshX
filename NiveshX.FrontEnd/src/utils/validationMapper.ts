...
export function mapServerErrorsToFieldErrors(
  ...
  if (
    ...
    Object.entries(data.errors as Record<string, unknown>).forEach(
      ([key, val]) => {
  ...
  if (data && "errors" in data && Array.isArray(data.errors)) {
    (data.errors as any[]).forEach((it) => {
      ...
      const msg = String(
        it?.message ?? it?.msg ?? it?.error ?? JSON.stringify(it)
      );
  ...
  if (data && (data.title || (data as any).detail || data.message)) {
    if (data.title) {
      result["__global"] = (result["__global"] || []).concat(
        String(data.title)
      );
    }
  ...
  if (
    ...
    try {
      for (const [k, v] of Object.entries(
        ...
        if (Array.isArray(v)) {
          (v as unknown[]).forEach(
            (m) =>
              (result[normalizedKey] = (result[normalizedKey] || []).concat(
                String(m)
              ))
          );
        } else {
